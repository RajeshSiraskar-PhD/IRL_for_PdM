...
%% V.1.2: 20-Sep-2024. Added Osa. Timeline. 03-Oct-2024. Added business part in intro.
%% V.1.3: 05-Oct-2024: Added Refs to .bib and begin SLR section
%% -------------------------------------------------------------------------------
...
In this section we conduct a detailed study of previous research on applying IL to the domain of PdM.
...
\begin{table}[h!]
	\renewcommand{\arraystretch}{1.5}
	\fontfamily{lmss}\selectfont
	\begin{tabular}{L{2.5cm} L{4cm} L{4cm} L{4cm}}
		\midrule[0.01pt]
		\textbf{Article} & \textbf{Application area} & \textbf{Technique} & \textbf{Novelty and comments}\\ \midrule[0.01pt]
		\cite{PdM-Hao2024} & Operation and maintenance policy for IoT (cyber physical) based energy systems.  & Proximal Policy Optimization (PPO) and Behavioral Cloning & Cyber aging as a SDP. Behavioral Cloning to generate trajectories and then train RL maintenance agent using PPO. \\
		\cite{PdM-Hao2023} & xxxxx & xxx & xxx\\
		\cite{PdM-Sugisawa2022} & xxxx \\
		\cite{PdM-Oh2019} & xxxx & xxx & xxx\\
		\midrule[0.01pt]
	\end{tabular}
	\caption{Research articles that have applied Imitation Learning techniques to predictive maintenance.} \label{tbl:IL-PdM}
\end{table}
\endgroup

\subsubsection{Case-study-1: \cite{PdM-Hao2024}}
\cite{PdM-Hao2024} -- \citefield{PdM-Hao2024}{title}.

Operation and maintenance policy for IoT (cyber physical) based energy systems.

Proximal Policy Optimization (PPO) and Behavioral Cloning and Cyber aging as a SDP. Behavioral Cloning to generate trajectories and then train RL maintenance agent using PPO. The cyber-aging model considers memory leakages and resultant data-jamming. They are modeled as a Continuous-Time Markov Chain (CTMC) based on \citep{Hao2021-CPES-1, Hao2021-CPES-2}.

RE-WORD below copied points\\
1. case study conducted on Advanced Lead-cooled Fast Reactor European Demonstrator (ALFRED)\\
2. The main hardware components of ALFRED, i.e., water pump, sensors, turbine admission valve and control rods, are considered equipped with RUL estimation capabilities,\\
3. cyber component (controller) is considered able to access current available memory and operating time\\
4. Tech stack: PPO + IL (Behavioral Cloning) + CPES model ``that embeds the hardware components RULs estimator, the hardware components failure process model and the cyber aging model''\\
5. Cyber aging caused by aging-related bugs (such as memory leakage) generate errors that propagate inside the system. Memory leakage may lead to data-jamming and prevent data processing or tasks delivering in due time, ultimately resulting in data queueing and memory request increase that blocks the system [25], reduces the controllability and stability of the controlled physical system, and leads to system failure [34]. \\
  


\subsubsection{Case-study-2: \cite{PdM-Hao2023}}
\cite{PdM-Hao2023} -- \citefield{PdM-Hao2023}{title}
xx

\subsubsection{Case-study-3: \cite{PdM-Sugisawa2022}}
\cite{PdM-Sugisawa2022} -- \citefield{PdM-Sugisawa2022}{title}
xx

\subsubsection{Case-study-4: \cite{PdM-Oh2019}}
\cite{PdM-Oh2019} -- \citefield{PdM-Oh2019}{title}
xx
...
%	\renewcommand{\arraystretch}{1.5}
%	\fontfamily{lmss}\selectfont
%	\begin{tabular}{L{2cm} L{6cm} L{3cm} L{3cm}}
%		\midrule[0.01pt]
%		\textbf{Article} & \textbf{Title} & \textbf{Application area} & \textbf{Technique}\\ \midrule[0.01pt]
%		\cite{PdM-Hao2024} & \citefield{PdM-Hao2024}{title} & xxx & Proximal Policy Optimization (PPO) and
...
%		\midrule[0.01pt]
%	\end{tabular}
%	\caption{Research articles that have applied Imitation Learning techniques to predictive maintenance.} \label{tbl:-del-IL-PdM}
%\end{table}
%\endgroup
	...
	\begin{center}
		\begin{minipage}{\textwidth}
			...
			\begin{tabular}{@{} l L{4cm} l L{4cm} @{}}
				...
				SDP & Sequential Decision Problem&PHM& Prognostic Health Management\\
