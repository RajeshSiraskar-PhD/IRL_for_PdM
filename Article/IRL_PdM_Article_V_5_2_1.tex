%% MOHOPADA-H-PC work 09-Sep 10:00am
%% NOTES: Stanford lecs used capital Xi \Xi for demonstrations, I've used \mathcal{D} 
%% Transfered from H-PC MOHOPADA 08-Sep. 10:20 am
...
\begin{algorithm}\label{algorithm:DAgger}
	\onehalfspacing
	\caption{DAgger Dataset Aggregation algorithm, \cite{stanford-lectures}} 
	\begin{algorithmic}[1]
		\State {Data: $\pi^*$}
		\State {Result: $\hat{\pi}^*$}
		\State {Initialize $\mathcal{D} \leftarrow 0$}
		\State {Initialize $\hat{\pi}$}
		\For {i=1 to N}
			\State {$\pi_i = \beta_i \pi^* + (1 - \beta_i) \hat{\pi}$}
			\State {Rollout policy $\pi_i$ to sample trajectory $\tau = \{s_0, s_1, ..., s_T\}$ }
			\State {Query expert $\pi^*$ and generate dataset $\mathcal{D}_i = \{(s_0, \pi^*(s_0)), (s_1, \pi^*(s_1)), ..., (s_T, \pi^*(s_T))\}$}
			\State {Aggregate datasets: $\mathcal{D} \leftarrow \mathcal{D} \cup \mathcal{D}_i $}
			\State {Retrain policy $\hat{\pi}$ using aggregated dataset $\mathcal{D}$}
		\EndFor
		\State {Return $\hat{\pi}$}
	\end{algorithmic} 
\end{algorithm}
...
Notation: $\Pi$ is the set of policies the learner is considering and $T$ the time horizon of the task.

\begin{algorithm}\label{algorithm:DAgger-Ross}
	\onehalfspacing
	\caption{DAgger Dataset Aggregation algorithm, \cite{Dagger-Ross2010}} 
	\begin{algorithmic}[1]
		...
		\For {i=1 to N}
			...
			\State {and actions given by expert}
			\State {Aggregate datasets: $\mathcal{D} \leftarrow \mathcal{D} \cup \mathcal{D}_i$}
			\State {Train policy $\hat{\pi}_{i+1}$ on $\mathcal{D}$}
		\EndFor
		\State {Return best $\hat{\pi}_i$ on validation.}
	\end{algorithmic} 
\end{algorithm}

% $$$ Additional content from YUE and LE 2018 -- \citep{yue2018imitation}
%
